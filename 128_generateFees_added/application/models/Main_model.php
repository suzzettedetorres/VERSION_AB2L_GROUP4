...
class Main_model extends CI_Model{
	...
	public function add_pr_unpaidfee($ufid, $account, $month, $paymentrecordid){
		$this->db->query("Insert into CMSC128.PAYMENTRECORD_UNPAIDFEE(UFID, ACCOUNT, MONTH, PAYMENTRECORDID) values ('$ufid', '$account', '$month', '$paymentrecordid')");
	}
	
	public function add_pr_paidfee($pfid, $date, $month, $maccount, $prid){
		$this->db->query("Insert into CMSC128.PAYMENTRECORD_PAIDFEE(PFID, DATEOFPAYMENT, MONTH, MONTHACCOUNT, PAYMENTRECORDID) values ('$pfid', '$date', '$month', '$maccount', '$prid')");
	}
	...
	public function add_dormer_equipment($stnum, $controlnum,  $date_borrowed, $date_returned, $condbeforeborrowed, $condafterreturned){
		$this->db->query("Insert into CMSC128.DORMER_EQUIPMENT(STUDENTNUMBER, CONTROLNUMBER, DATEBORROWED, DATERETURNED, CONDBEFOREBORROWED, CONDAFTERRETURNED) values ('$stnum', '$controlnum', '$date_borrowed', '$date_returned', '$condbeforeborrowed', '$condafterreturned')");
	}
	...
	*/
	...
	public function checkIfHasPaidBefore($paymentrecordid){
		return $this->db->query("select LAST_DAY(DATEOFPAYMENT) from PAYMENTRECORD_PAIDFEE where PAYMENTRECORDID = '$paymentrecordid'")->result_array();
	}
