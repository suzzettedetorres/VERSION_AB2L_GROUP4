...
<html>
<head>
<title>Remove Dormer's Appliance Record</title>
...
<body onload="startTime()">	
    ...
    <div id="content">
    	<header>
            <h1>Remove Dormer's Appliance Record</h1>
        </header>
        <section>
...
<form id="removeAppliance" name="removeAppliance" method="post" action="remove_appliance" onsubmit="">
	<?php
		foreach($data as $d){
			print $d['LASTNAME'].', '.$d['FIRSTNAME'].' '.$d['MIDDLENAME'].'<br/>';
			print $d['STUDENTNUMBER'];
			print '<input type="hidden" name="stnum" value="'.$d['STUDENTNUMBER'].'">';
		}
	?>
	<table>
	<tbody>
	<?php
		...
		foreach($appliances as $a){
			...
			print  '<tr><td><input type="radio" name="control_number" value="'.$a['CONTROLNUMBER'].'">'.$a['APPLIANCE'].'</td></tr>';
			print  '<tr><td><input type="hidden" name="appliance" value="'.$a['APPLIANCE'].'">';
		}
	?>
	
	<td colspan='2'>
	<input type="submit" value="Remove Appliance" onclick="" />
	</td>
	</tr>
	</table>
</form>
